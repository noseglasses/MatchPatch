...
def build_toolbar(window: MainWindowLike) -> None:
    ...
    window.save_measurement_action.setToolTip("Save the measurement Helix file for normalization.")
    ...
    window.run_normalization_action = QAction(_normalization_icon(), "Normalize", object_parent)
    window.run_normalization_action.setToolTip("Start the guided preset-normalization workflow.")
    window.run_normalization_action.setProperty("help_id", HelpId.NORMALIZE_SETLIST)
    window.run_normalization_action.triggered.connect(window.start_normalization)
    ...
    window.exit_action = QAction(
        window.style().standardIcon(QStyle.StandardPixmap.SP_DialogCloseButton),
        "Exit",
        object_parent,
    )
    window.exit_action.setToolTip("Close MatchPatch.")
...
def _build_menu_bar(window: MainWindowLike) -> None:
    parent = cast(QWidget, window)
    menu_bar = window.menuBar()

    window.file_menu = QMenu("File", parent)
    menu_bar.addMenu(window.file_menu)
    for action in (
        window.open_action,
        window.save_action,
        window.save_as_action,
        ...
        window.file_menu.addAction(action)
    window.file_menu.addSeparator()
    window.file_menu.addAction(window.split_setlist_action)
    window.file_menu.addSeparator()
    window.file_menu.addAction(window.exit_action)

    window.run_menu = QMenu("Run", parent)
    menu_bar.addMenu(window.run_menu)
    window.run_menu.addAction(window.run_normalization_action)

    window.help_menu = QMenu("Help", parent)
    menu_bar.addMenu(window.help_menu)
    window.help_menu.addAction(window.help_action)
